...
- **E-Mail-Migration** (`email-migration-tab.js`): **`POST …/imap-sync/start`**, **`GET …/jobs`**, **`GET …/config`** (health), **`DELETE …/imap-sync/jobs/{id}`**; proxy **`inc/routes/imap-sync-proxy.php`**, option **`dls_imap_sync_base_url`**. Stella: passfiles + `/health`.
...
**Documentation**
- [ ] Add the field to `acf-field-map.mdc` under the correct CPT section (ACF type + return value / notes)
