...
- **E-Mail-Migration** (`email-migration-tab.js`): **`POST /dls/v1/imap-sync/start`** starts sync on Stella; **`GET /dls/v1/imap-sync/jobs`** polls for the job list and progress (no client-side queue). Proxy + option `dls_imap_sync_base_url`: **`inc/routes/imap-sync-proxy.php`**. Separate from Nachrichten DB import.
