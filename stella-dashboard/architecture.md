...
- **E-Mail-Migration** (`email-migration-tab.js`): queues local tasks, starts jobs via **`POST /dls/v1/imap-sync/start`**; **`GET /dls/v1/imap-sync/jobs`** polls the Stella `imapsync` service for live status (progress, folders, message counts). Proxy + option `dls_imap_sync_base_url`: **`inc/routes/imap-sync-proxy.php`**. Separate from Nachrichten DB import.
